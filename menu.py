...
class admission:
      ...
      def feedetail(self):
            print("\t\t\tPending Fees ")
            query="select ID,NAME,PEFEE from studentadm"
            try:
                  cur=self.con.cursor()
                  cur.execute(query)
            except:
      ...
      def sfee(self):
            ...
            query="select ID,NAME,STD,PEFEE from studentadm where ID=%s"
            try:
                  cur=self.con.cursor()
                  cur.execute(query,self.id)
            except:
      ...
      def pay(self):
            ...
            print("\t\t\tTotal Fees are : 60000 ")
            self.p=int(input("\t\t\t Enter Amount for Payment : "))
            self.bal=60000 - self.p
            print("\t\t\tPending Fees are : ",self.bal)
            query="update studentadm set PEFEE=%s where ID=%s"
            cur=self.con.cursor()
